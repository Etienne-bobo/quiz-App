<template>
  <v-app>
    ...
    <v-main>
      ...
      <v-container>
        <v-btn fixed right @click="confirmationDialog = true" color="red white--text"> Delete </v-btn>
         ...
         <v-row justify="center">
          <v-dialog v-model="confirmationDialog" persistent max-width="600px">
            <v-card>
              ...
              <v-card-text>
               <p>This qction will remove definitively <b>{{quiz.name}}</b> quiz</p>
              </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="blue darken-1" text @click="confirmationDialog = false">
                  Close
                </v-btn>
        ...
        <v-row no-gutters v-if="quiz.questions.length != 0">
              <v-col v-for="(question, id) in quiz.questions" :key="id" cols="12" sm="4">
